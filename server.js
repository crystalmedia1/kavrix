...
// --- 1. DE SUPER PROXY (Omzeilt alle blokkades) ---
app.get("/proxy-m3u", async (req, res) => {
    ...
    try {
        // Poging 1: Directe fetch vanaf de server
        console.log("Poging 1: Directe fetch voor", url);
        const response = await axios.get(url, config);
        res.send(response.data);
    } catch (error) {
        console.log("Poging 1 mislukt, overschakelen naar AllOrigins...");
        try {
            // Poging 2: Via AllOrigins proxy als backup
            ...
            console.error("Alle pogingen mislukt voor:", url);
            res.status(500).send("Kon de lijst niet ophalen. Controleer of de link nog werkt.");
        }
    }
});

// --- 2. HULPFUNCTIE: SCHONE CODE FILTER ---
function cleanCode(text) {
    if (!text) return "";
    let code = text.trim();
    // Verwijder alle AI-praatjes en markdown
...
// --- 3. DE AI ARCHITECT (Met harde IPTV wetten) ---
app.post("/generate", async (req, res) => {
    ...
    // We vertellen de AI exact welke URL hij moet gebruiken voor de proxy
        ...
        const response = await axios.post(
            "https://api.groq.com/openai/v1/chat/completions",
            {
                ...
                messages: [
                    ...
                    { role: "user", content: existingCode ? `WIJZIG DEZE CODE: ${existingCode}\n\nOPDRACHT: ${prompt}` : `BOUW NIEUWE APP: ${prompt}` }
                ],
                temperature: 0.1
            },
            {
