...
const ProjectSchema = new mongoose.Schema({
  ...
  ai_debug_response: { type: String, default: '' } // trimmed raw AI response for debugging
});
...
// --------------------
// HELPERS: JSON repair & FRONTEND ASSET INJECTION
// --------------------

// Probeer broken JSON te herstellen: verwijder code-fences, trailing-commas, quote keys, enz.
function repairJsonFromText(text) {
  if (!text || typeof text !== 'string') return null;
  ...
  let candidate = objMatch ? objMatch[0] : (arrMatch ? arrMatch[0] : t);

  // Remove trailing commas before } or ]
  candidate = candidate.replace(/,\s*(}|])/g, '\$1');

  // Attempt to quote unquoted keys: `{ key: ` -> `{"key": `
  ...
  try {
    ...
    // Last resort: try to wrap into an object if it looks like naked html/css/js parts
    try {
      // If candidate doesn't parse, attempt to extract quoted "html": ... patterns
      const htmlMatch = t.match(/"html"\s*:\s*("([\s\S]*?)"|`([\s\S]*?)`)/i);
      const cssMatch = t.match(/"css"\s*:\s*("([\s\S]*?)"|`([\s\S]*?)`)/i);
      const jsMatch = t.match(/"js"\s*:\s*("([\s\S]*?)"|`([\s\S]*?)`)/i);
      if (htmlMatch || cssMatch || jsMatch) {
        const out = {
          html: htmlMatch ? (htmlMatch[2] || htmlMatch[3] || '') : '',
          css: cssMatch ? (cssMatch[2] || cssMatch[3] || '') : '',
          js: jsMatch ? (jsMatch[2] || jsMatch[3] || '') : ''
        };
        return out;
      }
    } catch (_) {}
...
// Inject frontend dependencies (Tailwind, Lucide, Motion/Animate) if missing
function ensureFrontendLibs(parsed, logoUrl, heroUrl) {
  parsed.html = parsed.html || '';
  parsed.js = parsed.js || '';
  parsed.css = parsed.css || '';

  // Add Tailwind + Lucide + Motion/Animate only if not present
  const needTailwind = !/cdn\.tailwindcss\.com|tailwindcss/i.test(parsed.html);
  const needLucide = !/lucide(@|\.|createIcons)/i.test(parsed.html + parsed.js);
  const needAnimate = !/animate\.css|motionone|@motionone/i.test(parsed.html + parsed.js);

  let headInjection = '';
  if (needTailwind) headInjection += `<script src="https://cdn.tailwindcss.com"></script>\n`;
  if (needLucide) headInjection += `<script src="https://unpkg.com/lucide@latest/dist/lucide.min.js"></script>\n`;
  // Motion One (lightweight) for JS animations
  if (needAnimate) headInjection += `<script src="https://cdn.jsdelivr.net/npm/@motionone/dom/dist/motion.js"></script>\n`;
  // Small CSS fallback for subtle animations
  if (headInjection) parsed.html = headInjection + parsed.html;

  // Ensure logo and hero usage hints: do not expose prompt/assets, but ensure they are reachable
  // If parsed HTML contains placeholders like {{LOGO}} or {{HERO}}, replace them
  if (logoUrl && parsed.html.includes('{{LOGO}}')) parsed.html = parsed.html.replace(/\{\{LOGO\}\}/g, logoUrl);
  if (heroUrl && parsed.html.includes('{{HERO}}')) parsed.html = parsed.html.replace(/\{\{HERO\}\}/g, heroUrl);

  // Ensure lucide icon init in JS
  parsed.js = `try{ if(window.lucide) lucide.createIcons(); }catch(e){}\n` + parsed.js;
...
Genereer een single-page app met HTML, CSS en JS in JSON-formaat. Zorg dat de UI geen zichtbare kopieën van de opdracht of asset-lijst toont. Gebruik de opgegeven assets (logo en foto).`;
    ...
    (async () => {
      try {
        ...
        // Save raw AI response for debugging (trim to a reasonable size)
        const rawAiData = JSON.stringify(aiResponse.data || {}).slice(0, 20000);

        // extract candidate text (support multiple response shapes)
        let text = null;
        if (aiResponse.data) {
        ...
        // Try multiple parsing strategies (auto-repair)
        let parsed = safeJSONParse(text);
        let attempts = 0;
        while (!parsed && attempts < 3) {
          attempts++;
          // try to repair typical issues
          const repaired = repairJsonFromText(text);
          if (repaired) {
            parsed = repaired;
            break;
          }
          // if not reparable, try to strip markdown fences and re-parse
          const stripped = (text || '').replace(/```(json|html)?/g, '').trim();
          const candidateMatch = stripped.match(/\{[\s\S]*\}|\[[\s\S]*\]/);
          if (candidateMatch) {
            try {
              parsed = JSON.parse(candidateMatch[0]);
              break;
            } catch (_) {}
          }
          // if still not, reduce to fallback attempt (break to avoid infinite loop)
          break;
        }

        // If still not parsed, produce graceful fallback but keep debug info
        if (!parsed) {
          console.warn('AI response not valid JSON after repair attempts. Storing raw for debugging.');
          const fallbackImg = chosenBackground || (fallbacks && fallbacks[0]) || '';
          parsed = {
            html: `<div style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:#111827;color:#fff;padding:40px;font-family:Inter,system-ui,sans-serif;"><div style="max-width:900px;text-align:center;">${fallbackImg ? `<img src="${buildProxyUrl(fallbackImg)}" alt="hero" style="max-width:100%;border-radius:12px;margin-bottom:20px"/>` : ''}<h1 style="font-size:32px;margin-bottom:8px">Gegenereerde App</h1><p>AI response kon niet correct geparsed worden. Check debug field.</p></div></div>`,
            css: '',
            js: '',
        ...
        try {
          ...
          let hasChosenBg = false;
          for (const c of candidates) { if (!c) continue; if (lower.includes(c.toLowerCase())) { hasChosenBg = true; break; } }
          if (!hasChosenBg) {
          ...
          if (!hasLogo) {
            const displayName = escapeHtml((prompt || '').split(' ')[0] || 'Kavrix');
            const logoHeader = `<header style="display:flex;align-items:center;gap:12px;padding:16px 20px;"><img src="${logo}" alt="Logo" style="width:48px;height:48px;border-radius:8px;object-fit:cover;"/><div style="font-weight:700">${displayName}</div></header>`;
            if (/<body[^>]*>/i.test(parsed.html)) {
              parsed.html = parsed.html.replace(/<body[^>]*>/i, match => `${match}\n${logoHeader}\n`);
            } else {
        ...
        // Ensure frontend libs (Tailwind, Lucide, MotionOne) are present and init icons
        parsed = ensureFrontendLibs(parsed, logo, chosenBackground);
        ...
        // save project (include verbatim_prompt field for auditing and raw AI response for debugging)
        await Project.findByIdAndUpdate(project._id, {
          files: {
            html: parsed.html,
            css: parsed.css,
          ...
          ai_debug_response: (rawAiData || '').toString().slice(0, 20000),
          updatedAt: new Date()
        }).exec();
